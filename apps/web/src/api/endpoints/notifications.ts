...
export const fetchUnreadNotificationSummary = async (_userId: string): Promise<NotificationUnreadSummary> => {
  const data = await apiJsonRequest<NotificationUnreadSummary>("/notifications/me/unread-summary", {
    method: "GET",
    auth: true,
  });
  ...
  return {
    ...
    unreadDisputes: Number(data?.unreadDisputes || 0),
  };
};
