...
const Order: React.FC = () => {
  ...
  const [appliedDiscountType, setAppliedDiscountType] = useState<'none' | 'weekly' | 'monthly'>('none')
  ...
  // Calculate total rental days and apply appropriate discount
  const rentalCalculation = useMemo(() => {
    if (!listing?.price.rent) {
      return { totalDays: 0, effectiveRate: 0, discountApplied: 'none' as const, rentSubtotal: 0 };
    }

    const dailyRate = listing.price.rent.daily;
    const weeklyRate = listing.price.rent.weekly;
    const monthlyRate = listing.price.rent.monthly;

    // Calculate total days
    let totalDays = rentalDurationCount;
    if (durationUnit === 'week') {
      totalDays = rentalDurationCount * 7;
    } else if (durationUnit === 'month') {
      totalDays = rentalDurationCount * 30;
    }

    // Determine which rate to apply based on total days
    let effectiveRate = dailyRate;
    let discountApplied: 'none' | 'weekly' | 'monthly' = 'none';

    if (totalDays >= 30) {
      effectiveRate = monthlyRate / 30; // Rate per day
      discountApplied = 'monthly';
    } else if (totalDays >= 7) {
      effectiveRate = weeklyRate / 7; // Rate per day
      discountApplied = 'weekly';
    }

    const rentSubtotal = Math.round(effectiveRate * totalDays);

    return { totalDays, effectiveRate, discountApplied, rentSubtotal };
  }, [listing, durationUnit, rentalDurationCount])

  // Update applied discount type when calculation changes
  useEffect(() => {
    setAppliedDiscountType(rentalCalculation.discountApplied);
  }, [rentalCalculation.discountApplied])

  const rentSubtotal = selectedMode === 'rent' ? rentalCalculation.rentSubtotal : 0
  const rentRateByUnit = rentalCalculation.effectiveRate // Keep for reference in order submission
          ...
          <div className="space-y-5">
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="space-y-3 text-sm text-gray-700">
                ...
                {selectedMode === 'rent' && appliedDiscountType !== 'none' ? (
                  <div className="flex items-center justify-between rounded-lg bg-green-50 px-2 py-1 text-green-700">
                    <span className="text-sm">
                      {appliedDiscountType === 'weekly' ? '✓ Weekly discount applied' : '✓ Monthly discount applied'}
                    </span>
                  </div>
                ) : null}
                {selectedMode === 'rent' ? (
