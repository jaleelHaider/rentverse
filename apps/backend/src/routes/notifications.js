...
notificationsRouter.get("/me/unread-summary", requireAuth, async (req, res) => {
  const userId = req.auth.sub;

  const { data, error, count } = await supabaseAdmin
    .from("notifications")
    .select("type,data", { count: "exact" })
    .eq("user_id", userId)
    .is("read_at", null)
    .limit(500);

  if (error) {
    return res.status(400).json({ message: error.message || "Failed to load notification summary." });
  }

  const rows = data || [];
  const disputeUnread = rows.filter((row) => {
    const type = String(row?.type || "");
    const kind = String(row?.data?.kind || "");
    return type.includes("dispute") || kind.startsWith("dispute_");
  }).length;

  return res.json({
    unreadTotal: count || 0,
    unreadDisputes: disputeUnread,
  });
});
